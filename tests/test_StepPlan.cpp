...
// a 100% coast move
TEST(Generator, CoastOnlyUnlimited)
{
	Generator3d generator(xLimits, yLimits, zLimits);
	Vec3Int32 start = {0, 0, 0};
	Vec3Int32 end = {100, 0, 0};
	Vector<uint16_t> startingSpeedDirection({500, 0, 0});
	uint16_t requestedSpeed = 500;
	uint16_t endingSpeed = 0;
	...
	for (size_t i = 0; i < steps.size(); ++i)
	{
		...
		uint16_t expectedMinDelay = 1.0 / 500 * 1000000;

		ASSERT_LE(actualMinDelay, expectedMinDelay);
	}
}
